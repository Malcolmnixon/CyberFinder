...
public class CharacterCameraConstraintGood : MonoBehaviour
{
	/// <summary>
	...
	[Tooltip("Objects causing fade")]
    public LayerMask FadeCollider;
	...
	void PreCharacterMovement()
	{
		...
		var headColliders = Physics.OverlapSphere(headPos, FadeMaxDistance, FadeCollider.value, QueryTriggerInteraction.Ignore);
        if (headColliders.Any())
        {
            var headDist = Mathf.Sqrt(headColliders.Min(c => (c.ClosestPoint(headPos) - headPos).sqrMagnitude));
            if (EnableFadeout)
            {
                var fadeLevel = 1.0F - Mathf.Clamp01((headDist - FadeMinDistance) / (FadeMaxDistance - FadeMinDistance));
                OVRScreenFade.instance?.SetExplicitFade(fadeLevel * MaxFade);
            }
		}
	}
}
